...
// Persistent lastKnownLocations and cabsRef for robustness
const lastKnownLocationsRef = typeof window !== "undefined" ? { current: new Map() } : { current: new Map() }
const cabsRef = typeof window !== "undefined" ? { current: [] } : { current: [] }

// Track component mount for safe state updates
const isMountedRef = typeof window !== "undefined" ? { current: true } : { current: true }
...
const GoogleMapsComponent = ({ cabs, selectedCab, onCabSelect }) => {
  ...
  useEffect(() => {
    ...
    const loadGoogleMaps = () => {
      ...
      const apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY || "AIzaSyAKjmBSUJ3XR8uD10vG2ptzqLJAZnOlzqI"
  ...
  const createMarkers = () => {
    ...
    cabs.forEach((cab) => {
      ...
      // Use current location, fallback to sessionStorage, then lastKnownLocationsRef
      ...
      if (
        ...
        if (!currentLocation) {
          currentLocation = lastKnownLocationsRef.current.get(cab.id)
        }
      } else {
        lastKnownLocationsRef.current.set(cab.id, currentLocation)
      ...
      const getVehicleIcon = (status, ignition, isSelected, heading = 0) => {
        ...
        if (isSelected) {
          ...
          if (!ignition) {
            ...
            iconUrl = "https://maps.google.com/mapfiles/kml/shapes/parking_lot.png"
          } else {
          ...
          <button
            onClick={() => {
              if (mapInstanceRef.current && selectedCab && isGoogleMapsLoaded) {
                const location =
                  selectedCab.location || getStoredLocation(selectedCab.imei) || lastKnownLocationsRef.current.get(selectedCab.id)
                if (location) {
  ...
  const initializeWebSocket = () => {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        const validCabs = (cabsRef.current || []).filter((cab) => cab?.imei)
        ...
        validCabs.forEach((cab) => {
          ...
          try {
            if (ws.readyState === WebSocket.OPEN) {
              ...
              console.warn("WebSocket not open when trying to subscribe to", cab.imei)
            }
          } catch (err) {
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          // Parse GPS data from TCP server
          const gpsLat = parseFloat(gpsData.latitude || gpsData.lat)
          const gpsLon = parseFloat(gpsData.longitude || gpsData.lon)
          const gpsSpeed = gpsData.speed !== undefined ? parseInt(gpsData.speed, 10) : 0
          const gpsHeading = parseFloat(gpsData.heading || gpsData.course || 0)
          const gpsIgnition = gpsData.ignition === "1" || gpsData.ignition === true || false
          if (!gpsData.imei || Number.isNaN(gpsLat) || Number.isNaN(gpsLon)) {
          ...
          setCabs((prevCabs) => {
            return prevCabs.map((cab) => {
              if (cab.imei === gpsData.imei) {
                const newLocation = { lat: gpsLat, lng: gpsLon }
                try {
                  sessionStorage.setItem(`location_${cab.imei}`, JSON.stringify({ ...newLocation, timestamp: Date.now() }))
                } catch (e) {}
                lastKnownLocationsRef.current.set(cab.id, newLocation)
                const ignition = gpsData.ignition !== undefined ? Boolean(gpsData.ignition) : cab.ignition
                let newStatus = "Parked"
                if (!ignition) newStatus = "Idle"
                else if (gpsSpeed > 5) newStatus = "Moving"
                else if (gpsSpeed > 0) newStatus = "Idle"
                return { ...cab, location: newLocation, speed: gpsSpeed, ignition, status: newStatus, heading: gpsData.heading || 0, lastUpdated: new Date().toISOString(), hasGPSData: true }
              }
              return cab
            })
          })
        } catch (err) {
          console.error('ws onmessage parse error', err, event.data)
        }
      }
      ws.onclose = (event) => {
        clearTimeout(connectionTimeout)
